...
func main() {
	App.WmTitle("Measure Standard and System Fonts")
...
func insertFontFamilies(t *TextWidget) {
	...
	for i, family := range getFontFamilies() {
		...
		t.TagConfigure(tag, Font(font))
		size := font.Measure(App, text)
		t.Insert(END, fmt.Sprintf("%5dpx in %s: ", size, family), "", text, tag)
	}
}
